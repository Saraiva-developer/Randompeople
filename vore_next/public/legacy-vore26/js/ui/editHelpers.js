...
function resolveEditorTabType(editor, tabId) {
...
export function getDraftSectionsUi(ctx, tabId) {
  ...
  const keys = tabKeyMap(resolveEditorTabType(editor, tabId));
...
export function blankItemUi(tabId) {
  if (tabId === "servicos") return { name: "", imageUrl: "", images: [], serviceType: "", serviceTypeLabel: "", time: "", price: "", quoteOnly: "no", promoEnabled: "no", promoOldPrice: "", promoNowPrice: "", shortDescription: "", description: "", note: "", extraFields: [], enabled: true };
...
export function renderEditItemCardUi(ctx, editTab, item, idx, options = {}) {
  ...
  const serviceTypeOptions = Array.isArray(options.serviceTypeOptions) ? options.serviceTypeOptions : [];
  ...
  const titleLabel = editTab === "servicos" ? "Nome do servico" : "Titulo";
  card += "<label>" + titleLabel + "<input class=\"input\" data-item-field=\"name\" data-item-idx=\"" + idx + "\" value=\"" + esc(t) + "\" /></label>";
  if (editTab === "servicos" || editTab === "produtos" || editTab === "menu" || editTab === "portfolio" || editTab === "casas" || editTab === "quartos") {
    ...
    card += "</div>";
    card += "</div>";
    if (isProductTab || editTab === "servicos") {
  ...
  if (editTab === "servicos") {
    ...
    const serviceTypeValue = typeMeta && typeMeta.id !== "general" && rawServiceType.toLowerCase() === typeMeta.id ? typeMeta.label : rawServiceType;
    const serviceTypeListId = "service-type-options-" + idx;
    card += "<label>Duracao<input class=\"input\" data-item-field=\"time\" data-item-idx=\"" + idx + "\" value=\"" + esc(tm) + "\" /></label>";
    card += "<label>Sob orcamento<select class=\"input\" data-item-field=\"quoteOnly\" data-item-idx=\"" + idx + "\"><option value=\"no\"" + (!quoteOnly ? " selected" : "") + ">Nao</option><option value=\"yes\"" + (quoteOnly ? " selected" : "") + ">Sim</option></select></label>";
    if (!quoteOnly) card += "<label>Preco (€)<input class=\"input\" data-item-field=\"price\" data-item-idx=\"" + idx + "\" value=\"" + esc(p) + "\" placeholder=\"Ex: 45 €\" /></label>";
    ...
    if (promoEnabled && !quoteOnly) {
      card += "<label>Preco antigo (€)<input class=\"input\" data-item-field=\"promoOldPrice\" data-item-idx=\"" + idx + "\" value=\"" + esc(String(item?.promoOldPrice || "")) + "\" placeholder=\"Ex: 50 €\" /></label>";
      card += "<label>Preco promocao (€)<input class=\"input\" data-item-field=\"promoNowPrice\" data-item-idx=\"" + idx + "\" value=\"" + esc(String(item?.promoNowPrice || "")) + "\" placeholder=\"Ex: 30 €\" /></label>";
    }
    card += "<label>Resumo curto<textarea class=\"input\" data-item-field=\"shortDescription\" data-item-idx=\"" + idx + "\" maxlength=\"220\" placeholder=\"Frase curta para aparecer no card do perfil\">" + esc(String(item && (item.shortDescription || item.summary) || "")) + "</textarea></label>";
    card += "<label>Descricao completa<textarea class=\"input\" data-item-field=\"description\" data-item-idx=\"" + idx + "\" placeholder=\"Detalhes que aparecem no modal do servico\">" + esc(String(item && item.description || "")) + "</textarea></label>";
    card += "<details class=\"edit-item-advanced\">";
    card += "<summary>Avancado</summary>";
    card += "<div class=\"edit-item-advanced-body\">";
    card += "<label>Tipo de servico<input class=\"input\" data-item-field=\"serviceType\" data-item-idx=\"" + idx + "\" value=\"" + esc(serviceTypeValue) + "\" list=\"" + esc(serviceTypeListId) + "\" placeholder=\"Ex: Massagem, Beleza, Terapia\" /></label>";
    card += "<datalist id=\"" + esc(serviceTypeListId) + "\">" + serviceTypeOptions.map((type) => "<option value=\"" + esc(type) + "\"></option>").join("") + "</datalist>";
    card += "<label>Nota interna / destaque<textarea class=\"input\" data-item-field=\"note\" data-item-idx=\"" + idx + "\">" + esc(String(item && (item.note || item.notes) || "")) + "</textarea></label>";
    card += "<p class=\"muted\">Blocos livres do modal</p>";
  ...
  if (editTab !== "servicos" && editTab !== "produtos" && editTab !== "portfolio" && editTab !== "campanhas") {
    ...
    card += "<label>" + priceLabel + "<input class=\"input\" data-item-field=\"" + priceField + "\" data-item-idx=\"" + idx + "\" value=\"" + esc(p) + "\" placeholder=\"Ex: 45 €\" /></label>";
  }
  if (isProductTab) {
    card += "<div class=\"edit-product-key-grid\">";
    card += "<label>Preco (€)<input class=\"input\" data-item-field=\"price\" data-item-idx=\"" + idx + "\" value=\"" + esc(p) + "\" placeholder=\"Ex: 19,90 €\" /></label>";
    ...
    card += "<label>Promocao<select class=\"input\" data-item-field=\"promoEnabled\" data-item-idx=\"" + idx + "\"><option value=\"no\"" + (!promoEnabled ? " selected" : "") + ">Desativada</option><option value=\"yes\"" + (promoEnabled ? " selected" : "") + ">Ativada</option></select></label>";
    if (promoEnabled) {
      card += "<label>Preco antigo (€)<input class=\"input\" data-item-field=\"promoOldPrice\" data-item-idx=\"" + idx + "\" value=\"" + esc(String(item?.promoOldPrice || "")) + "\" placeholder=\"Ex: 29,90 €\" /></label>";
      card += "<label>Preco promocao (€)<input class=\"input\" data-item-field=\"promoNowPrice\" data-item-idx=\"" + idx + "\" value=\"" + esc(String(item?.promoNowPrice || "")) + "\" placeholder=\"Ex: 19,90 €\" /></label>";
    }
    card += "</div>";
  }
  if (editTab === "menu" || editTab === "casas" || editTab === "quartos") {
