...
export function mapProfileRow(row) {
  ...
  const numericId = Number((row && row.id) || 0);
  ...
  return {
    id: numericId,
    remoteId: numericId,
    remoteUuid: String((row && (row.remote_id || row.remoteId)) || ''),
    userId: Number((row && row.user_id) || 0),
    userUuid: String((row && (row.user_uuid || row.userUuid)) || ''),
    ...
    isPublished: row && typeof row.is_published === 'boolean' ? row.is_published : true,
...
export function profileSections(profile, mainTab) {
  ...
  const matchedTab = tabs.find((tab) => String((tab && (tab.id || tab.type)) || "").toLowerCase() === String(mainTab || "").toLowerCase());
  const tabType = String((matchedTab && (matchedTab.type || matchedTab.id)) || mainTab || "").toLowerCase();
  let source = [];
  if (tabType === 'servicos') source = d.servicesSections || d.services;
  else if (tabType === 'produtos') source = d.productsSections || d.products;
  else if (tabType === 'menu') source = d.menuSections || d.menu;
  else if (tabType === 'portfolio') source = d.portfolioSections || d.portfolio;
  else if (tabType === 'casas') source = d.housesSections || d.houses;
  else if (tabType === 'quartos') source = d.roomsSections || d.rooms;
  else if (tabType === 'campanhas') source = d.campaignSections || d.campaigns;
  else if (tabType === 'galeria') {
  ...
  if (tabType === 'servicos' || tabType === 'produtos' || tabType === 'menu') {
