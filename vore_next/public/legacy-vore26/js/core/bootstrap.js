export async function bootstrapUi(ctx) {
  ...
  try {
    ...
    let profiles = ((feed && feed.profiles) || []).map(mapProfileRow);
    if (me && me.authenticated && me.user && String(me.user.account_type || "").toLowerCase() !== "common") {
      const authUserId = Number(me.user.id || 0);
      const ownProfile = profiles.find((p) => Number((p && p.userId) || 0) === authUserId) || null;
      if (!ownProfile) {
        const displayName = String(me.user.name || me.user.email || "Perfil").trim() || "Perfil";
        ...
        if (created && created.profile) {
          profiles = [mapProfileRow(created.profile), ...profiles];
        }
      }
    }
    const ownProfileForUser = me && me.authenticated && me.user
      ? profiles.find((p) => Number((p && p.userId) || 0) === Number(me.user.id || 0))
      : null;
    setState({ profiles, selectedProfileId: (ownProfileForUser || profiles[0]) ? (ownProfileForUser || profiles[0]).id : null });
    if (me && me.authenticated && me.user && String(me.user.account_type || "").toLowerCase() === "common") {
