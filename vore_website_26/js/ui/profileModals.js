...
function renderItemModal() {
  ...
  const body = root.querySelector("#itemModalBody");
  if (body && body.classList) body.classList.toggle("gallery-mode", isGalleryModal);
  const panelEl = root.querySelector(".item-modal-panel");
  if (panelEl && panelEl.classList) panelEl.classList.toggle("gallery-mode", isGalleryModal);
  const articleClass = isGalleryModal ? "item-modal-gallery" : "panel";
  body.innerHTML = (
    "<article class=\"" + articleClass + "\">" +
      (isGalleryModal ? "" : ("<p class=\"muted\">" + esc(String(itemModalState.index + 1) + " / " + String(itemModalState.items.length)) + "</p>")) +
      ...
      modalHead +
      (isGalleryModal ? (mediaBlock + thumbsBlock) : (thumbsBlock + mediaBlock)) +
      (subtitle && !isGalleryModal ? "<p class=\"muted\">" + esc(subtitle) + "</p>" : "") +
      serviceDetailHtml +
