...
export function renderEditScreen(ctx) {
  ...
  if (Number(editor.autosaveHydratedFor || 0) !== Number(profile.id)) {
    ...
    if (payload && payload.snapshot) {
      ...
      editor.lastAutosaveAt = Number(payload.updatedAt || 0) || 0;
    }
  ...
  if (editTab === "sobre") {
    ...
    const addItemLabel =
      editTab === "servicos" ? "Adicionar servico" :
      editTab === "produtos" ? "Adicionar produto" :
      editTab === "menu" ? "Adicionar item" :
      editTab === "portfolio" ? "Adicionar projeto" :
      editTab === "campanhas" ? "Adicionar campanha" :
      (editTab === "casas" || editTab === "quartos") ? "Adicionar local" : "Adicionar item";
  ...
  const renderLivePreview = () => {
    ...
    const previewTabId = (previewTabs.find((tab) => tab.id === previewSelected) || previewTabs.find((tab) => tab.id === editTab) || previewTabs[0] || { id: "sobre" }).id;
    editor.previewNav.tabId = previewTabId;
    let previewSections = [];
    if (previewTabId !== "sobre") {
      if (isSimpleEditTab(previewTabId)) {
        previewSections = buildSimplePreviewSections(previewTabId, editor, getDraftGalleryLists);
      } else {
        previewSections = getDraftSections(previewTabId);
        if (previewTabId === "campanhas") {
    ...
    const visibleItems = rawItems.filter((item) => isEnabledFlag(item && item.enabled));
    ...
    if (previewTabId === "sobre") {
    ...
    } else if (previewTabId === "produtos" || previewTabId === "menu") {
      const isProducts = previewTabId === "produtos";
      ...
      const cardsHtml = currentView === "grid"
        ? (
          typeof buildCatalogGridCard === "function"
            ? visibleItems.map((item, idx) => buildCatalogGridCard(previewTabId, item, idx)).join("")
            : visibleItems.map((item, idx) => renderItem(previewTabId, item, idx)).join("")
        )
        : (
          typeof buildCatalogListCard === "function"
            ? visibleItems.map((item, idx) => buildCatalogListCard(previewTabId, item, idx)).join("")
            : visibleItems.map((item, idx) => renderItem(previewTabId, item, idx)).join("")
        );
      ...
      const wrapperClass = getPreviewContentWrapperClass(previewTabId);
    ...
    previewRoot.innerHTML =
      "<div class=\"edit-preview-shell\">" +
        ...
        (previewTabId === "sobre" || !previewSections.length ? "" : "<div class=\"chips edit-preview-subtabs\">" + previewSections.map((section) => {
          const key = section.id || section.label;
  ...
  const sanitizeShortLabel = (value, maxLength) => {
  ...
  if (addLinkBtn) {
    addLinkBtn.addEventListener("click", () => {
      const type = String((editor.newLinkDraft && editor.newLinkDraft.type) || "").trim().toLowerCase();
      const url = sanitizeLinkInputUrl((editor.newLinkDraft && editor.newLinkDraft.url) || "");
      const label = sanitizeShortLabel((editor.newLinkDraft && editor.newLinkDraft.label) || "", LINK_LABEL_MAX_LENGTH);
      editor.newLinkDraft.url = url;
      editor.newLinkDraft.label = label;
      if (newLinkUrlField && String(newLinkUrlField.value || "") !== url) newLinkUrlField.value = url;
      if (newLinkLabelField && String(newLinkLabelField.value || "") !== label) newLinkLabelField.value = label;
      if (!type) {
        setEditorStatus("Seleciona a rede antes de adicionar.");
        if (newLinkTypeField) newLinkTypeField.focus();
        return;
      }
      if (!url) {
        setEditorStatus("Preenche o link antes de adicionar.");
        if (newLinkUrlField) {
          applyUrlFieldValidity(newLinkUrlField, type, "");
          newLinkUrlField.focus();
        }
        return;
      }
      if (!isLikelyUrlForLink(url, type)) {
        setEditorStatus("Este link parece invalido para " + getLinkTypeLabel(type) + ".");
        if (newLinkUrlField) {
          applyUrlFieldValidity(newLinkUrlField, type, url);
          newLinkUrlField.focus();
        }
        return;
      }
      ...
      if (duplicate) {
        ...
        if (newLinkUrlField) newLinkUrlField.focus();
        return;
      }
      next.push({
    ...
    el.edit.querySelectorAll("[data-item-field]").forEach((field) => {
      const applyField = () => {
        ...
        if (key === "promoEnabled" || key === "quoteOnly") {
          ...
          section.items[idx].serviceType = normalizedType;
          section.items[idx].type = normalizedType;
