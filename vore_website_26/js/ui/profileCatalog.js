...
export function renderProfileLodgingTabUi(ctx, tabId, sections, subId) {
  ...
  const gallery = currentEntry
    ? (() => {
      ...
      const sideEntries = alternateEntries.slice(0, 2);
      const stripEntries = alternateEntries.slice(2, 7);
      const remainingCount = Math.max(0, alternateEntries.length - sideEntries.length - stripEntries.length);
      const sideHtml = sideEntries.map((entry, slotIndex) => (
        "<button type=\"button\" class=\"profile-lodging-gallery-side-item\" data-lodging-thumb=\"" + entry.idx + "\">" +
          "<img src=\"" + esc(entry.entry.url) + "\" alt=\"foto " + String(slotIndex + 2) + "\" />" +
        "</button>"
      )).join("");
      const stripHtml = stripEntries.map((entry, slotIndex) => (
        "<button type=\"button\" class=\"profile-lodging-gallery-strip-item\" data-lodging-thumb=\"" + entry.idx + "\">" +
          "<img src=\"" + esc(entry.entry.url) + "\" alt=\"thumb " + String(slotIndex + 1) + "\" />" +
        "</button>"
      )).join("");
      const moreTile = remainingCount > 0
        ? ("<button type=\"button\" class=\"profile-lodging-gallery-strip-item profile-lodging-gallery-more\" data-lodging-open-modal=\"1\">+" + remainingCount + " fotos</button>")
        : "";
      return (
        "<div class=\"profile-lodging-gallery profile-lodging-gallery-mosaic\">" +
          "<button type=\"button\" class=\"profile-lodging-gallery-main\" data-lodging-thumb=\"" + currentEntry.idx + "\" data-lodging-open-modal=\"1\">" +
            "<img class=\"profile-lodging-gallery-main-image\" src=\"" + esc(currentEntry.entry.url) + "\" alt=\"" + esc(title) + "\" />" +
          "</button>" +
          "<div class=\"profile-lodging-gallery-side\">" + (sideHtml || "<div class=\"profile-lodging-gallery-side-placeholder\"></div>") + "</div>" +
        "</div>" +
        ((stripHtml || moreTile) ? "<div class=\"profile-lodging-gallery-strip\">" + stripHtml + moreTile + "</div>" : "")
      );
    })()
